...
The runner copies the read-only Git worktree into an ephemeral container filesystem, executes the policy argv, bounds runtime and output, redacts common credential forms, and writes an atomic JSON artifact plus SHA-256 manifest. The report includes start/end timestamps, exit and timeout state, output checksum, bundle checksum, local artifact reference, container digest, and command argv. The control plane recomputes both checksums, rejects metadata that differs from the run policy snapshot, and rejects output that still resembles a raw credential.
