...
| `runner` | Separately versioned local/self-hosted execution client; it uses lease-bound GitHub App credentials to clone or refresh connected repositories, runs isolated worktrees, containers, tests, and browsers, and invokes allowlisted local MCP stdio context tools. |
