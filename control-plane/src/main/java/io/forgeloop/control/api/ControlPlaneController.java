...
public class ControlPlaneController {
  private final FeatureRunService runs; private final RepositoryConnectionService connections; private final RunnerService runners; private final OrganizationService organizations; private final OperatorContext operators; private final AuditLedgerService audit; private final GithubDeliveryService githubDelivery; private final GithubInstallationRepositorySyncService installationSync; private final GithubInstallationRepository githubInstallations; private final VerificationEvidenceRepository evidence; private final GithubPublicationRepository publications;
  public ControlPlaneController(FeatureRunService runs, RepositoryConnectionService connections, RunnerService runners, OrganizationService organizations, OperatorContext operators, AuditLedgerService audit, GithubDeliveryService githubDelivery, GithubInstallationRepositorySyncService installationSync, GithubInstallationRepository githubInstallations, VerificationEvidenceRepository evidence, GithubPublicationRepository publications) { this.runs = runs; this.connections = connections; this.runners = runners; this.organizations = organizations; this.operators = operators; this.audit = audit; this.githubDelivery = githubDelivery; this.installationSync = installationSync; this.githubInstallations = githubInstallations; this.evidence = evidence; this.publications = publications; }
  ...
  @MutationMapping public FeatureRun approveFeatureRun(@Argument String runId, @Argument String confirmation) { operators.requireAdministrator(); return runs.approve(runId, operators.subject(), confirmation); }
