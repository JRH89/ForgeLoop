...
public class FeatureRun {
  ...
  public void addPolicyVerificationTasks(){List<DeliveryTask> prerequisites=tasks.stream().filter(task->!"PLANNER".equals(task.getRole())&&!"VERIFICATION".equals(task.getRole())).toList();for(VerificationGate gate:gates){if(gate.getKind()==null)continue;if(!gate.isRequired()){gate.skipByPolicy();continue;}DeliveryTask verification=new DeliveryTask(this,"verify-"+gate.getName(),"VERIFICATION","Verify "+gate.getName(),"docker",List.of(),2,0);verification.attachVerificationGate(gate);prerequisites.forEach(verification::dependsOn);tasks.add(verification);}}
  ...
  public void approve(String actor){if(state!=RunState.READY_FOR_REVIEW)throw new IllegalStateException("Only a verified run can be approved");if(approvedAt!=null)throw new IllegalStateException("Run is already approved");if(actor==null||actor.isBlank())throw new IllegalArgumentException("Approver is required");approvedAt=Instant.now();approvedBy=actor;}
  public void resumeAfterRetry(){if(state!=RunState.BLOCKED&&state!=RunState.FAILED)throw new IllegalStateException("Run is not blocked");state=RunState.EXECUTING;approvedAt=null;approvedBy=null;}
